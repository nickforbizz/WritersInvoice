...
export default async function handler(req, res) {
    ...
    switch (method) {
        case 'GET':
            try {
                const response = await CompleteOrder.findByPk(id);
        ...
        case 'PUT':
            try {
                let response = await CompleteOrder.findByPk(id);
                ...
                await CompleteOrder.update(req.body,{ where: { id,  }});
                response = await CompleteOrder.findByPk(id);
        ...
        case 'DELETE':
            try {
                ...
                await CompleteOrder.destroy({ where: { id: id }});
