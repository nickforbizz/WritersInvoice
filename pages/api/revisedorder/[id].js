...
const {RevisedOrder} = require('../../../models');
...
export default async function handler(req, res) {
    ...
    switch (method) {
        case 'GET':
            try {
                const response = await RevisedOrder.findByPk(id);
        ...
        case 'PUT':
            try {
                let response = await RevisedOrder.findByPk(id);
                ...
                await RevisedOrder.update(req.body,{ where: { id,  }});
                response = await RevisedOrder.findByPk(id);
        ...
        case 'DELETE':
            try {
                ...
                await RevisedOrder.destroy({ where: { id: id }});
