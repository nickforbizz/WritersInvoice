...
export default async function handler(req, res) {
    ...
    switch (method) {
        case 'GET':
            try {
                const response = await CancelledOrder.findByPk(id);
        ...
        case 'PUT':
            try {
                let response = await CancelledOrder.findByPk(id);
                ...
                await CancelledOrder.update(req.body,{ where: { id,  }});
                response = await CancelledOrder.findByPk(id);
        ...
        case 'DELETE':
            try {
                ...
                await CancelledOrder.destroy({ where: { id: id }});
